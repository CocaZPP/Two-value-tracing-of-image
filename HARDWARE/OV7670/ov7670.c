...
u8 OV7670_Init(void)
{
	...
	RCC->AHB1ENR|=1<<1;		//先使能外设PORTB时钟
  RCC->AHB1ENR|=1<<2;		//先使能外设PORTC时钟
	RCC->AHB1ENR|=1<<3;		//先使能外设PORTD时钟
	RCC->AHB1ENR|=1<<4;		//先使能外设PORTE时钟	   
	RCC->AHB1ENR|=1<<6;		//先使能外设PORTG时钟

//帧中断	
	GPIO_Set(GPIOA,PIN8,GPIO_MODE_IN,0,0,GPIO_PUPD_PD); 			//PA8设置为下拉输入 
	GPIOA->ODR|=1<<8; 
	
//RCK 
	GPIO_Set(GPIOA,PIN6,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU); //PA6设置输出
	GPIOA->ODR|=1<<6; 

	
		GPIO_Set(GPIOA,PIN13,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU); //PD9设置输出
	GPIOA->ODR|=1<<13; 
//WEN
	GPIO_Set(GPIOG,PIN9,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU); //PD9设置输出
	GPIOG->ODR|=1<<9; 
	
//WRST 	
	GPIO_Set(GPIOB,PIN7,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU); //PB7设置输出
 	GPIOB->ODR|=1<<7;

// RRST
	GPIO_Set(GPIOA,PIN4,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU); //PA4设置输出
 	GPIOA->ODR|=1<<4;
	
//CS
	GPIO_Set(GPIOG,PIN15,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU); //PG15设置输出
	GPIOG->ODR=1<<15;      
	
//DATA	
	GPIO_Set(GPIOC,PIN6|PIN7|PIN8|PIN9|PIN11,GPIO_MODE_IN,0,0,GPIO_PUPD_PD); 			//PC6 7 8 9 11设置为下拉输入 	
	GPIOC->ODR|=0x0BC0;                                                           // D0 1 2 3 4

	GPIO_Set(GPIOB,PIN6,GPIO_MODE_IN,0,0,GPIO_PUPD_PD); 			//PB6设置为下拉输入 	
	GPIOB->ODR|=1<<6;                                         // D5
	
	GPIO_Set(GPIOE,PIN5|PIN6,GPIO_MODE_IN,0,0,GPIO_PUPD_PD); 			//PE5 6设置为下拉输入 	
